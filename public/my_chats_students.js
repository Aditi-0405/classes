...
const display_chats = async () => {
  ...
  if (token) {
    ...
    console.log("response chats: ", response1.data)

    const response_chats = response1.data.user
    console.log("response chats user",response_chats)
    ...
    response_chats.chat.forEach(element => {
      ...
      sender.innerHTML = `<a href= "/see_individual_chat_student?from=${element.senderId}&to=${user_id}" class= "link">
